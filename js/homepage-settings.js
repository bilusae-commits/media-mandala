...
(function(){
  ...
  function setImage(id,value,alt){const x=document.getElementById(id);if(x&&value){x.src=value;x.alt=alt||x.alt}}
  ...
  function applySettings(s){
    ...
    const hero=document.getElementById("heroSection");if(hero){if(s.hero_background_color)hero.style.backgroundColor=s.hero_background_color;if(s.hero_overlay_image){hero.style.backgroundImage="linear-gradient(rgba(255,255,255,"+Math.max(0,Math.min(1,Number(s.hero_overlay_opacity??.18)))+"),rgba(255,255,255,"+Math.max(0,Math.min(1,Number(s.hero_overlay_opacity??.18)))+")),url(\""+String(s.hero_overlay_image).replace(/\"/g,"%22")+"\")";hero.style.backgroundSize="cover";hero.style.backgroundPosition="center"}}
  }
  function applyLatest(article){
    if(!article)return;
    const feature=document.querySelector(".feature");
    if(feature){const a=feature.querySelector("a")||feature;const img=feature.querySelector("img"),title=feature.querySelector("h2"),meta=feature.querySelector(".k"),small=feature.querySelector("small");if(img&&article.cover_image_url)img.src=article.cover_image_url;if(title)title.textContent=article.title||"";if(meta)meta.textContent="Berita terbaru";if(small)small.textContent="Mandala Channel · Terbaru";if(a&&a.tagName==="A")a.href="pages/artikel-detail.html"+(article.slug?"?slug="+encodeURIComponent(article.slug):"")}
    const cards=document.querySelectorAll("#articlesContainer .card");if(cards.length){const card=cards[0],img=card.querySelector("img"),title=card.querySelector("h3"),meta=card.querySelector(".meta"),link=card.querySelector("a");if(img&&article.cover_image_url)img.src=article.cover_image_url;if(title)title.textContent=article.title||"Artikel terbaru";if(meta)meta.textContent="Artikel terbaru";if(link)link.href="pages/artikel-detail.html"+(article.slug?"?slug="+encodeURIComponent(article.slug):"")}}
