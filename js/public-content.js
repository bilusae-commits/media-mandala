...
(function(){
  ...
  async function client(){
    ...
    if(!window.supabase||typeof window.supabase.createClient!=="function"){
      await new Promise((resolve,reject)=>{
        ...
        const s=document.createElement("script");s.src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2";s.dataset.mandalaSupabasePublic="true";s.onload=resolve;s.onerror=()=>reject(new Error("Supabase library gagal dimuat."));document.head.appendChild(s);
      });
    }
    db=window.supabase.createClient(URL,KEY,{auth:{persistSession:false,autoRefreshToken:false}});return db;
  }
  ...
  async function loadHomeData(){
    ...
    const data={articles:articles.filter(x=>x.status==="published"),videos:videos.filter(x=>x.status==="published"),playlists:playlists.filter(x=>x.status==="published"),topics:active,categories:active};
    window.DATA=data;window.MandalaPublicData=data;
    renderArticles(data.articles,map);renderVideos(data.videos,map);renderTopics(data.topics);renderPlaylistState(data.playlists);
    return data;
  }
  ...
  function renderVideos(items,map){
    const el=document.getElementById("videosContainer");if(!el||!items.length)return;
    el.innerHTML=items.slice(0,4).map(v=>{const id=v.youtube_video_id||youtubeId(v.youtube_url),cat=map[v.category_id]?.name||"VIDEO";return `<article class="card video"><a href="#" data-public-video="${esc(id)}" data-public-title="${esc(v.title)}"><div class="thumb"><img src="${esc(videoImage({...v,youtube_video_id:id}))}" alt="${esc(v.title)}" loading="lazy"><span class="badge">${esc(cat)}</span></div><div class="meta">VIDEO · ${esc(dateText(v.published_at||v.created_at))}</div><h3>${esc(v.title||"Video Mandala")}</h3><p>Putar video →</p></a></article>`;}).join("");
    el.querySelectorAll("[data-public-video]").forEach(a=>a.addEventListener("click",e=>{e.preventDefault();if(typeof window.openVideo==="function")window.openVideo(a.dataset.publicVideo,a.dataset.publicTitle);}));
  }

  function renderTopics(items){
    const el=document.getElementById("topicsContainer");if(!el||!items.length)return;
  ...
  function renderPlaylistState(items){
    const track=document.getElementById("playlistTrack");if(!track||items.length)return;
    track.innerHTML=`<div style="position:absolute;inset:0;display:grid;place-items:center;color:#8b8d89;font-size:12px;text-align:center;padding:20px">Belum ada playlist yang diterbitkan.</div>`;
    const dots=document.getElementById("playlistDots");if(dots)dots.innerHTML="";
  }
