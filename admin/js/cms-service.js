...
(function(window){
  ...
  if (!API) {
    console.error('MandalaSupabase belum dimuat. Pastikan ../js/supabase-client.js dimuat lebih dulu.');
    return;
  }
  ...
  async function getVideos(){
    ...
    const {data,error}=await db.from('videos').select('id,title,slug,youtube_url,youtube_video_id,thumbnail_url,description,category_id,status,featured,published_at,created_at,updated_at,author_id').order('created_at',{ascending:false});
  ...
  async function changeStatus(tableName,id,status){
