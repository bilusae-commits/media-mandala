...
function renderMedia() {
  ...
  mediaList.innerHTML = mediaRows.map((row, i) => {
    ...
    return `<div class="media-item" data-media-id="${escapeHtml(row.id)}"><img src="${escapeHtml(url)}" alt="${escapeHtml(row.alt_text || row.caption || `Foto ${i + 1}`)}" loading="lazy"><div class="media-fields"><input class="media-caption" value="${escapeHtml(row.caption || "")}" placeholder="Caption foto"><input class="media-alt" value="${escapeHtml(row.alt_text || "")}" placeholder="Alt text"><input class="media-order" type="number" min="1" max="${MAX_MEDIA}" value="${Math.min(MAX_MEDIA, Math.max(1, Number(row.sort_order) + 1 || i + 1))}" placeholder="Posisi"></div><div class="media-actions"><button class="btn media-insert" type="button" data-action="insert">Sisipkan</button><button class="btn media-insert" type="button" data-action="save">Simpan</button><button class="btn media-delete" type="button" data-action="delete">Hapus</button></div></div>`;
  }).join("");
