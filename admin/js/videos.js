...
function statusLabel(v) {
  return ({ draft: "Draft", review: "Review", published: "Published", archived: "Arsip" }[v] || v);
}
...
function filtered() {
  ...
  return videos.filter(v => {
    const text = [v.title, v.slug, v.youtube_video_id, v.youtube_url, v.description]
...
function render() {
  ...
  if (!rows.length) {
    container.innerHTML = '<div class="empty"><strong>Tidak ada video</strong>Sesuaikan pencarian atau filter, atau buat video baru.</div>';
    return;
  }
...
function row(v) {
  ...
  return `<tr>
    <td><div style="display:flex;gap:11px;align-items:center">
      <img class="thumb" src="${esc(image)}" alt="${esc(v.title)}" loading="lazy" onerror="this.style.visibility='hidden'">
      <div><div class="video-name">${esc(v.title || "Tanpa judul")}</div>
      <div class="video-sub">${esc(v.youtube_video_id || "YouTube ID belum ada")}${v.featured ? '<span class="badge badge-featured">★ Unggulan</span>' : ''}</div></div>
    </div></td>
    <td>${esc(c?.name || "-")}</td>
    <td><span class="badge badge-${esc(v.status || "draft")}">${esc(statusLabel(v.status || "draft"))}</span></td>
    <td>${date(v.created_at)}</td>
    <td>${v.published_at ? date(v.published_at) : "-"}</td>
    <td><div class="actions">
      <a class="action" href="video-edit.html?id=${encodeURIComponent(v.id)}">Edit</a>
      <button class="action danger" type="button" data-delete="${esc(v.id)}" ${canDelete ? "" : 'disabled title="Tidak dapat dihapus"'}>Hapus</button>
    </div></td>
  </tr>`;
}
...
async function load() {
  try {
    ...
    // Semua halaman admin sekarang menggunakan satu auth service yang sama.
    // Jika sesi benar-benar tidak ada, arahkan ke login, bukan kembali ke dashboard.
    ...
    console.error("Video Admin Load Error:", e);
    setMessage(e?.message || "Gagal memuat video.", "error");
    container.innerHTML = `<div class="empty"><strong>Gagal memuat video</strong>${esc(e?.message || "Terjadi kesalahan.")}</div>`;
  }
}
...
async function handleDelete(e) {
  ...
  if (v.status === "published" && auth?.role !== "admin") {
  ...
  try {
    ...
    b.disabled = false;
    b.textContent = "Hapus";
    setMessage(err?.message || "Gagal menghapus video.", "error");
  }
}
...
$("#logout").addEventListener("click", async () => {
  try {
    ...
    setMessage(e?.message || "Gagal keluar.", "error");
  }
});
